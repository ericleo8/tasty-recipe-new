...
export default {
    ...
    actions: {
        async getRegisterData({ commit, dispatch }, payload) {
            const APIkey = "AIzaSyDLyWO_A0lTECN_spwMjLzE4ljOef9NgUE";
            const authUrl =
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                const { data } = await axios.post(
                    `https://vue-new-project-baru-default-rtdb.firebaseio.com/user.json?auth=${state.token}`, payload);
        ...
        async getUser({ commit }, payload) {
            try {
                const { data } = await axios.get(
                    `https://vue-new-project-baru-default-rtdb.firebaseio.com/user.json`)
                for (let key in data) {
